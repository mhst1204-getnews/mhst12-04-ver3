...
$array_structure_image[''] = "news";
$array_structure_image['Y'] = "news" . '/' . date('Y');
$array_structure_image['Ym'] = "news" . '/' . date('Y_m');
$array_structure_image['Y_m'] = "news" . '/' . date('Y/m');
$array_structure_image['Ym_d'] = "news" . '/' . date('Y_m/d');
$array_structure_image['Y_m_d'] = "news" . '/' . date('Y/m/d');
$array_structure_image['username'] = "news" . '/' . $username_alias;
$array_structure_image['username_Y'] = "news" . '/' . $username_alias .
    '/' . date('Y');
$array_structure_image['username_Ym'] = "news" . '/' . $username_alias .
    '/' . date('Y_m');
$array_structure_image['username_Y_m'] = "news" . '/' . $username_alias .
    '/' . date('Y/m');
$array_structure_image['username_Ym_d'] = "news" . '/' . $username_alias .
    '/' . date('Y_m/d');
$array_structure_image['username_Y_m_d'] = "news" . '/' . $username_alias .
    '/' . date('Y/m/d');
$structure_upload = isset($module_config["news"]['structure_upload']) ? $module_config["news"]['structure_upload'] :
    "Ym";
...
{
    $upload_real_dir_page = NV_UPLOADS_REAL_DIR . '/news';
...
$uploads_dir_user = NV_UPLOADS_DIR . '/news';
if (!defined('NV_IS_SPADMIN') and strpos($structure_upload, 'username') !== false)
{
    ...
    {
        $uploads_dir_user = NV_UPLOADS_DIR . '/news/' . $username_alias;
    }
}
$array_block_cat_module = array();
$id_block_content = array();
$sql = "SELECT bid, adddefault, title FROM `" . NV_PREFIXLANG . "_news_block_cat` ORDER BY `weight` ASC";
...
$array_imgposition = array(0 => $lang_module['imgposition_0'], 1 => $lang_module['imgposition_1'],
    2 => $lang_module['imgposition_2']);
$rowcontent = array("id" => "", "catid" => $catid, "listcatid" => "" . $catid .
    "," . $parentid . "", "topicid" => "", "admin_id" => $admin_id, "author" => "",
    "sourceid" => 0, "addtime" => NV_CURRENTTIME, "edittime" => NV_CURRENTTIME,
    "status" => 0, "publtime" => NV_CURRENTTIME, "exptime" => 0, "archive" => 1,
    "title" => "", "alias" => "", "hometext" => "", "sourcetext" => "",
    "homeimgfile" => "", "homeimgalt" => "", "homeimgthumb" => "", "imgposition" =>
    1, "bodyhtml" => "", "copyright" => 0, "inhome" => 1, "allowed_comm" => $module_config["news"]['setcomm'],
    "allowed_rating" => 1, "allowed_send" => 1, "allowed_print" => 1, "allowed_save" =>
    1, "hitstotal" => 0, "hitscm" => 0, "total_rating" => 0, "click_rating" => 0,
    "keywords" => "");
...
if ($rowcontent['id'] > 0)
{
    ...
    $rowcontent = $db->sql_fetchrow($db->sql_query("SELECT * FROM `" . NV_PREFIXLANG .
        "_news_rows` where `id`=" . $rowcontent['id']), 2);
            ...
            foreach ($arr_catid as $catid_i)
            {
                if (isset($array_cat_admin[$admin_id][$catid_i]))
                {
                    ...
                    } else
                    {
                        ...
                        {
                            ++$check_edit;
                        } elseif ($array_cat_admin[$admin_id][$catid_i]['pub_content'] == 1 and ($status ==
                        0 or $status = 2))
                        {
                            ++$check_edit;
    ...
    $rowcontent['topictext'] = "";
    $body_contents = $db->sql_fetch_assoc($db->sql_query("SELECT * FROM `" .
        NV_PREFIXLANG . "_news_bodyhtml_" . ceil($rowcontent['id'] /
        2000) . "` where `id`=" . $rowcontent['id']));
    $rowcontent = array_merge($rowcontent, $body_contents);
    unset($body_contents);
    $id_block_content = array();
    $sql = "SELECT bid FROM `" . NV_PREFIXLANG . "_news_block` where `id`='" . $rowcontent['id'] . "' ";
    $result = $db->sql_query($sql);
...
$array_cat_add_content = $array_cat_pub_content = $array_cat_edit_content =
    array();
...
if ($nv_Request->get_int('save', 'post') == 1)
{
    $catids = array_unique($nv_Request->get_typed_array('catids', 'post', 'int',
        array()));
    $id_block_content = array_unique($nv_Request->get_typed_array('bids', 'post',
        'int', array()));
    ...
    if ($rowcontent['topicid'] == 0)
    {
        ...
        {
            list($rowcontent['topicid']) = $db->sql_fetchrow($db->sql_query("SELECT `topicid` FROM `" .
                NV_PREFIXLANG . "_news_topics` WHERE `title`=" . $db->dbescape($rowcontent['topictext'])));
        }
    }
    ...
    if (!empty($publ_date) and preg_match("/^([0-9]{1,2})\\/([0-9]{1,2})\/([0-9]{4})$/",
        $publ_date, $m))
    {
    ...
    if (!empty($exp_date) and preg_match("/^([0-9]{1,2})\\/([0-9]{1,2})\/([0-9]{4})$/",
        $exp_date, $m))
    {
    ...
    //$alias=$_POST['alias'];
    $rowcontent['alias'] = ($alias == "") ? change_alias($rowcontent['title']) :
        change_alias($alias);
    ...
    $rowcontent['bodyhtml'] = defined('NV_EDITOR') ? nv_nl2br($bodyhtml, '') :
        nv_nl2br(nv_htmlspecialchars(strip_tags($bodyhtml)), '<br />');
    ...
    $rowcontent['allowed_rating'] = (int)$nv_Request->get_bool('allowed_rating',
        'post');
    $rowcontent['allowed_send'] = (int)$nv_Request->get_bool('allowed_send', 'post');
    $rowcontent['allowed_print'] = (int)$nv_Request->get_bool('allowed_print',
        'post');
    ...
    if (empty($error))
    {
        $rowcontent['catid'] = in_array($rowcontent['catid'], $catids) ? $rowcontent['catid'] :
            $catids[0];
        $rowcontent['bodytext'] = nv_news_get_bodytext($rowcontent['bodyhtml']);
        if (!empty($rowcontent['topictext']) and empty($rowcontent['topicid']))
        {
            list($weightopic) = $db->sql_fetchrow($db->sql_query("SELECT max(`weight`) FROM `" .
                NV_PREFIXLANG . "_news_topics`"));
            $weightopic = intval($weightopic) + 1;
            $aliastopic = change_alias($rowcontent['topictext']);
            $query = "INSERT INTO `" . NV_PREFIXLANG . "_news_topics` (`topicid`, `title`, `alias`, `description`, `image`, `thumbnail`, `weight`, `keywords`, `add_time`, `edit_time`) VALUES (NULL, " .
                $db->dbescape($rowcontent['topictext']) . ", " . $db->dbescape($aliastopic) .
                ", " . $db->dbescape($rowcontent['topictext']) . ", '', '', " . $db->dbescape($weightopic) .
                ", " . $db->dbescape($rowcontent['topictext']) .
        ...
        if (!empty($rowcontent['sourcetext']))
        {
            ...
            if (isset($url_info['scheme']) and isset($url_info['host']))
            {
                ...
                list($rowcontent['sourceid']) = $db->sql_fetchrow($db->sql_query("SELECT `sourceid` FROM `" .
                    NV_PREFIXLANG . "_news_sources` WHERE `link`=" . $db->dbescape($sourceid_link)));
                if (empty($rowcontent['sourceid']))
                {
                    list($weight) = $db->sql_fetchrow($db->sql_query("SELECT max(`weight`) FROM `" .
                        NV_PREFIXLANG . "_news_sources`"));
                    $weight = intval($weight) + 1;
                    $query = "INSERT INTO `" . NV_PREFIXLANG . "_news_sources` (`sourceid`, `title`, `link`, `logo`, `weight`, `add_time`, `edit_time`) VALUES (NULL, " .
                        $db->dbescape($url_info['host']) . ", " . $db->dbescape($sourceid_link) .
                        ", '', " . $db->dbescape($weight) . ", UNIX_TIMESTAMP( ), UNIX_TIMESTAMP( ))";
                    $rowcontent['sourceid'] = $db->sql_query_insert_id($query);
                }
            } else
            {
                list($rowcontent['sourceid']) = $db->sql_fetchrow($db->sql_query("SELECT `sourceid` FROM `" .
                    NV_PREFIXLANG . "_news_sources` WHERE `title`=" . $db->dbescape
                ...
                if (empty($rowcontent['sourceid']))
                {
                    ...
                    $weight = intval($weight) + 1;
                    $query = "INSERT INTO `" . NV_PREFIXLANG . "_news_sources` (`sourceid`, `title`, `link`, `logo`, `weight`, `add_time`, `edit_time`) VALUES (NULL, " .
                        $db->dbescape($rowcontent['sourcetext']) . ", '', '', " . $db->dbescape($weight) .
                        ", UNIX_TIMESTAMP( ), UNIX_TIMESTAMP( ))";
                    $rowcontent['sourceid'] = $db->sql_query_insert_id($query);
                }
            }
        ...
        {
            $lu = strlen(NV_BASE_SITEURL . NV_UPLOADS_DIR . "/news/");
        ...
        if ($rowcontent['id'] > 0)
        {
            list($homeimgfile, $homeimgthumb) = $db->sql_fetchrow($db->sql_query("SELECT `homeimgfile`, `homeimgthumb` FROM `" .
                NV_PREFIXLANG . "_news_rows` WHERE `id`=" . $rowcontent['id']));
            if ($rowcontent['homeimgfile'] != $homeimgfile)
            {
                ...
                if ($homeimgthumb != "" and $homeimgthumb != "|")
                {
                    ...
                    {
                        if (file_exists(NV_ROOTDIR . '/' . NV_FILES_DIR . "/news/" . $homeimgthumb_i))
                        {
                            nv_deletefile(NV_ROOTDIR . '/' . NV_FILES_DIR . "/news/" . $homeimgthumb_i);
                        }
                    }
                }
            } else
        ...
        if ($check_thumb and file_exists($homeimgfile))
        {
            ...
            $i = 1;
            while (file_exists(NV_ROOTDIR . '/' . NV_FILES_DIR . '/news/thumb/' . $thumb_basename))
            {
            ...
            $image->save(NV_ROOTDIR . '/' . NV_FILES_DIR . '/news/thumb', $thumb_basename);
            $image_info = $image->create_Image_info;
            $thumb_name = str_replace(NV_ROOTDIR . '/' . NV_FILES_DIR . '/news/', '', $image_info['src']);
            $block_basename = $basename;
            $i = 1;
            while (file_exists(NV_ROOTDIR . '/' . NV_FILES_DIR . '/news/block/' . $block_basename))
            {
                $block_basename = preg_replace('/(.*)(\.[a-zA-Z]+)$/', '\1_' . $i . '\2', $basename);
                ++$i;
            }
            $image->resizeXY($module_config["news"]['blockwidth'], $module_config["news"]['blockheight']);
            $image->save(NV_ROOTDIR . '/' . NV_FILES_DIR . '/news/block', $block_basename);
            $image_info = $image->create_Image_info;
            $block_name = str_replace(NV_ROOTDIR . '/' . NV_FILES_DIR . '/news/', '', $image_info['src']);
            $image->close();
            $rowcontent['homeimgthumb'] = $thumb_name . "|" . $block_name;
        }
        if ($rowcontent['id'] == 0)
        {
            $rowcontent['publtime'] = ($rowcontent['publtime'] > NV_CURRENTTIME) ? $rowcontent['publtime'] :
                NV_CURRENTTIME;
            if ($rowcontent['status'] == 1 and $rowcontent['publtime'] > NV_CURRENTTIME)
            {
                $rowcontent['status'] = 2;
            }
            $query = "INSERT INTO `" . NV_PREFIXLANG . "_news_rows` \n\t\t\t\t(`id`, `catid`, `listcatid`, `topicid`, `admin_id`, `author`, `sourceid`, `addtime`, `edittime`, `status`, `publtime`, `exptime`, `archive`, `title`, `alias`, `hometext`, `homeimgfile`, `homeimgalt`, `homeimgthumb`, `inhome`, `allowed_comm`, `allowed_rating`, `hitstotal`, `hitscm`, `total_rating`, `click_rating`, `keywords`) VALUES \n                (NULL, \n                " .
                ...
                dbescape_string($rowcontent['alias']) . ",\n                " . $db->
                dbescape_string($rowcontent['hometext']) . ",\n                " . $db->
                dbescape_string($rowcontent['homeimgfile']) . ",\n                " . $db->
                dbescape_string($rowcontent['homeimgalt']) . ",\n                " . $db->
                dbescape_string($rowcontent['homeimgthumb']) . ",\n                " . intval($rowcontent['inhome']) .
            ...
            if ($rowcontent['id'] > 0)
            {
                nv_insert_logs(NV_LANG_DATA, $module_name, $lang_module['content_add'], $rowcontent['title'],
                    $admin_info['userid']);
                $ct_query = array();
                $tbhtml = NV_PREFIXLANG . "_news_bodyhtml_" . ceil($rowcontent['id'] /
                    2000);
                ...
                {
                    $ct_query[] = (int)$db->sql_query("INSERT INTO `" . NV_PREFIXLANG . "_news_" . $catid . "` SELECT * FROM `" . NV_PREFIXLANG . "_" . $module_data .
                        "_rows` WHERE `id`=" . $rowcontent['id'] . "");
                }
                $ct_query[] = (int)$db->sql_query("INSERT INTO `" . NV_PREFIXLANG . "_news_bodytext` VALUES (" . $rowcontent['id'] . ", " . $db->dbescape_string($rowcontent['bodytext']) .
                    ")");
        ...
        {
            $rowcontent_old = $db->sql_fetchrow($db->sql_query("SELECT * FROM `" .
                NV_PREFIXLANG . "_news_rows` where `id`=" . $rowcontent['id'] .
                ""));
            ...
            $query = "UPDATE `" . NV_PREFIXLANG . "_news_rows` SET \n                           `catid`=" .
                intval($rowcontent['catid']) . ", \n\t\t\t\t           `listcatid`=" . $db->
                dbescape_string($rowcontent['listcatid']) . ", \n                           `topicid`=" .
                intval($rowcontent['topicid']) . ", \n                           `author`=" . $db->
                dbescape_string($rowcontent['author']) . ", \n                           `sourceid`=" .
                intval($rowcontent['sourceid']) . ", \n                           `status`=" .
                intval($rowcontent['status']) . ", \n                           `publtime`=" .
                intval($rowcontent['publtime']) . ", \n                           `exptime`=" .
                intval($rowcontent['exptime']) . ", \n                           `archive`=" .
                intval($rowcontent['archive']) . ", \n                           `title`=" . $db->
                dbescape_string($rowcontent['title']) . ", \n                           `alias`=" .
            ...
            {
                nv_insert_logs(NV_LANG_DATA, $module_name, $lang_module['content_edit'], $rowcontent['title'],
                    $admin_info['userid']);
                $ct_query = array();
                $ct_query[] = (int)$db->sql_query("UPDATE `" . NV_PREFIXLANG . "_news_bodyhtml_" . ceil($rowcontent['id'] / 2000) . "` SET \n\t\t\t\t\t\t`bodyhtml`=" .
                    $db->dbescape_string($rowcontent['bodyhtml']) . ", \n                       \t`sourcetext`=" .
                    $db->dbescape_string($rowcontent['sourcetext']) . ", \n\t\t\t\t\t   \t`imgposition`=" .
                    intval($rowcontent['imgposition']) . ",\n                       \t`copyright`=" .
                    intval($rowcontent['copyright']) . ", \n                       \t`allowed_send`=" .
                ...
                {
                    $ct_query[] = (int)$db->sql_query("DELETE FROM `" . NV_PREFIXLANG . "_news_" . $catid . "` WHERE `id` = " . $rowcontent['id'] . "");
                }
                foreach ($array_cat_new as $catid)
                {
                    $ct_query[] = (int)$db->sql_query("REPLACE INTO `" . NV_PREFIXLANG . "_news_" . $catid . "` SELECT * FROM `" . NV_PREFIXLANG . "_news_rows` WHERE `id`=" . $rowcontent['id'] . "");
                }
                $ct_query[] = (int)$db->sql_query("UPDATE `" . NV_PREFIXLANG . "_" . $module_data .
                    "_bodytext` SET `bodytext`=" . $db->dbescape_string($rowcontent['bodytext']) .
                    " WHERE `id` =" . $rowcontent['id']);
                if (array_sum($ct_query) != sizeof($ct_query))
                {
                    $error[] = $lang_module['errorsave'];
        ...
        if (empty($error))
        {
            ...
            {
                $db->sql_query("INSERT INTO `" . NV_PREFIXLANG . "_news_block` (`bid`, `id`, `weight`) VALUES ('" . $bid_i . "', '" . $rowcontent['id'] .
                    "', '0')");
            }
            $id_block_content[] = 0;
            $db->sql_query("DELETE FROM `" . NV_PREFIXLANG . "_news_block` WHERE `id` = " . $rowcontent['id'] . " AND `bid` NOT IN (" . implode(",",
                $id_block_content) . ")");
...
if (!empty($rowcontent['homeimgfile']) and file_exists(NV_UPLOADS_REAL_DIR . "/news/" . $rowcontent['homeimgfile']))
{
    $rowcontent['homeimgfile'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . "/news/" . $rowcontent['homeimgfile'];
}
$array_catid_in_row = explode(",", $rowcontent['listcatid']);
$sql = "SELECT topicid, title FROM `" . NV_PREFIXLANG . "_news_topics` ORDER BY `weight` ASC";
$result = $db->sql_query($sql);
...
$sql = "SELECT sourceid, title FROM `" . NV_PREFIXLANG . "_news_sources` ORDER BY `weight` ASC";
$result = $db->sql_query($sql);
...
{
    Header("Location: " . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" .
        $module_name . "&" . NV_OP_VARIABLE . "=cat");
    die();
}
$contents = "";
$my_head = "<link rel=\"stylesheet\" type=\"text/css\" href=\"" .
    NV_BASE_SITEURL . "js/jquery/jquery.autocomplete.css\" />\n";
$my_head .= "<link type=\"text/css\" href=\"" . NV_BASE_SITEURL .
    "js/ui/jquery.ui.core.css\" rel=\"stylesheet\" />\n";
$my_head .= "<link type=\"text/css\" href=\"" . NV_BASE_SITEURL .
    "js/ui/jquery.ui.theme.css\" rel=\"stylesheet\" />\n";
$my_head .= "<link type=\"text/css\" href=\"" . NV_BASE_SITEURL .
    "js/ui/jquery.ui.datepicker.css\" rel=\"stylesheet\" />\n";
$my_head .= "<script type=\"text/javascript\" src=\"" . NV_BASE_SITEURL .
    "js/jquery/jquery.autocomplete.js\"></script>\n";
$my_head .= "<script type=\"text/javascript\" src=\"" . NV_BASE_SITEURL .
    "js/ui/jquery.ui.core.min.js\"></script>\n";
$my_head .= "<script type=\"text/javascript\" src=\"" . NV_BASE_SITEURL .
    "js/ui/jquery.ui.datepicker.min.js\"></script>\n";
$my_head .= "<script type=\"text/javascript\" src=\"" . NV_BASE_SITEURL .
    "js/language/jquery.ui.datepicker-" . NV_LANG_INTERFACE . ".js\"></script>\n";
$xtpl = new XTemplate("content.tpl", NV_ROOTDIR . "/themes/" . $global_config['module_theme'] .
    "/modules/" . $module_file);
...
foreach ($global_array_cat as $catid_i => $array_value)
{
    ...
    if (!empty($check_show))
    {
        ...
        $catiddisplay = (sizeof($array_catid_in_row) > 1 and (in_array($catid_i, $array_catid_in_row))) ?
            '' : ' display: none;';
        $temp = array('catid' => $catid_i, "space" => $space, "title" => $array_value['title'],
...
for ($i = 0; $i <= 23; ++$i)
{
    $select .= "<option value=\"" . $i . "\"" . (($i == $phour) ? " selected=\"selected\"" :
...
for ($i = 0; $i < 60; ++$i)
{
    $select .= "<option value=\"" . $i . "\"" . (($i == $pmin) ? " selected=\"selected\"" :
        "") . ">" . str_pad($i, 2, "0", STR_PAD_LEFT) . "</option>\n";
}
...
for ($i = 0; $i <= 23; ++$i)
{
    $select .= "<option value=\"" . $i . "\"" . (($i == $ehour) ? " selected=\"selected\"" :
        "") . ">" . str_pad($i, 2, "0", STR_PAD_LEFT) . "</option>\n";
}
...
for ($i = 0; $i < 60; ++$i)
{
    $select .= "<option value=\"" . $i . "\"" . (($i == $emin) ? " selected=\"selected\"" :
        "") . ">" . str_pad($i, 2, "0", STR_PAD_LEFT) . "</option>\n";
}
...
while (list($commid_i, $commid_title_i) = each($array_allowed_comm))
{
    $comm_sl = ($commid_i == $rowcontent['allowed_comm']) ? " selected=\"selected\"" :
        "";
    $select .= "<option value=\"" . $commid_i . "\" " . $comm_sl . ">" . $commid_title_i .
        "</option>\n";
}
...
while (list($sourceid_i, $source_title_i) = each($array_source_module))
{
    $source_sl = ($sourceid_i == $rowcontent['sourceid']) ? " selected=\"selected\"" :
        "";
    $select .= "<option value=\"" . $sourceid_i . "\" " . $source_sl . ">" . $source_title_i .
        "</option>\n";
}
$xtpl->assign('sourceid', $select);
if (defined('NV_EDITOR') and nv_function_exists('nv_aleditor'))
{
    $edits = nv_aleditor('bodyhtml', '100%', '300px', $rowcontent['bodyhtml'], $uploads_dir_user,
        $currentpath);
} else
{
    $edits = "<textarea style=\"width: 100%\" name=\"bodyhtml\" id=\"bodyhtml\" cols=\"20\" rows=\"15\">" .
        $rowcontent['bodyhtml'] . "</textarea>";
}
$shtm = "";
if (sizeof($array_block_cat_module))
{
    foreach ($array_block_cat_module as $bid_i => $bid_title)
    {
        ...
        $shtm .= "<tr><td><input class=\"news_checkbox\" type=\"checkbox\" name=\"bids[]\" value=\"" .
            $bid_i . "\"" . $ch . " />" . $bid_title . "</td></tr>\n";
    }
